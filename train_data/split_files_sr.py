...
# subdirectories
#tiles = 'data_tiles'
coastTiles = 'data_tiles_forest_segments_sr'
labels = 'data_labels_sr'
polys = 'data_labels_poly_sr'
...
for d in [trainCtilesDir, valCtilesDir, testCtilesDir, trainLabelsDir, valLabelsDir, testLabelsDir, trainPolysDir, valPolysDir, testPolysDir]:
    #trainTilesDir, valTilesDir, testTilesDir,
...
tile_ids = os.listdir(labelsDir)
tile_ids = [x.split('.')[0] for x in tile_ids]
...
for d in [coastTilesDir, labelsDir, polysDir]:
    if os.listdir(d) == []:
        shutil.rmtree(d)