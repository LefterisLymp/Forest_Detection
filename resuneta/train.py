...
def dice_coef_multilabel(y_true, y_pred, numLabels):
    ...
    for index in range(numLabels):
        dice += dice_coef_multi(y_true[:,:,:,index], y_pred[:,:,:,index])
...
logging.basicConfig(filename="resunet_a-training_forest_org_hybrid.log",
...
def train(epochs, batch_size, model_save_dir):
    ...
    for e in range(0, epochs + 1):
        ...
        # Y_pred = keras.utils.to_categorical(np.argmax(Y_pred, axis=3), num_classes=4)
...
model_save_dir = 'checkpoints_weighted_forest_org_tanimoto/'
